...
| Error | Cause | Fix |
|-------|-------|-----|
| Kernel crash loop | Old `os.kill` approach | Fixed — no restart needed in current version |
| `PySparkRuntimeError: JAVA_GATEWAY_EXITED` | Wrong `JAVA_HOME` | Fixed — dynamically detected via `readlink` |
| `AssertionError: zip not found` | Zip not at `/content/Assignment_4-_datasets.zip` | Upload via Colab file panel (folder icon, left sidebar) |
| `SparkContext already running` | Cell re-run | Handled by `try: sc.stop()` guard |
